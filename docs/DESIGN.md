...
- **加载态**: 数据解析期间显示一张轻量骨架卡片；加载完成后自动卸载，不影响注入 JSON 后的 PDF 导出。
- **首屏进入**: 报告抬头以 420ms 的淡入上浮出现，元数据徽标以 40ms 节奏依次进入。
- **滚动浮现**: 报告标题、内容分节和语句提升项使用浏览器原生 `IntersectionObserver` 触发一次性的淡入上浮；只动画 `opacity` 和 `transform`，避免滚动重排。
- **可访问与打印**: `prefers-reduced-motion: reduce` 下直接显示全部内容；打印样式会强制关闭动画并显示所有元素，因此 C# Playwright 直接生成 PDF 时无需等待动效结束。
- **依赖**: 不引入 GSAP 或任何新运行时包；构建后仍为 `dist/index.html` 加字体目录的静态部署结构。
